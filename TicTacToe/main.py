...
class Game:
    ...
    def result(self, state, action, player):
        ...
        cp_state[action[0]][action[1]] = player
...
def MaxValue(game, state, player):
    ...
    for a in game.actions(state):
        ...
        v2, a2 = MinValue(game, game.result(state_copy, a, p), player)
        if v2 > v:
            v, move = v2, a  
...
def MinValue(game, state, player):
    ...
    for a in game.actions(state):
        state_copy = deepcopy(state)
        v2, a2 = MaxValue(game, game.result(state_copy, a, p), player)
        if v2 < v:
            v, move = v2, a    
    return v, move
